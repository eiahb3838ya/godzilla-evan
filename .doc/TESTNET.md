...
# Account configuration (REPLACE with your keys)
config = {
...
# Insert account
cursor.execute('''
INSERT OR REPLACE INTO account_config (user_id, source_name, receive_td, config)
VALUES (?, ?, ?, ?)
''', ('gz_user1', 'binance', 1, json.dumps(config)))

conn.commit()
conn.close()
print("✅ Account 'gz_user1' added successfully")
EOF
```

**Replace these values**:
- `YOUR_API_KEY_HERE` → Your Binance Testnet API Key
- `YOUR_SECRET_KEY_HERE` → Your Binance Testnet Secret Key
...
''')
config = {'access_key': 'YOUR_KEY', 'secret_key': 'YOUR_SECRET'}
...
**Default Initialization** (`trader_binance.cpp` line 88):
```cpp
_start_userdata(InstrumentType::FFuture);  // Only initializes Futures on startup
```

- Spot connection only attempted during reconnection logic
- This is a design decision, not a bug
- Expected: "spot login failed" if using Futures-only key
