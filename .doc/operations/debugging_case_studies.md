...
本文檔記錄真實除錯案例的深度分析,供學習系統架構和除錯方法論使用。

**快速排查問題?** 請使用 [debugging_guide.md](debugging_guide.md) (操作手冊)
...
**快速跳轉**:
- 如果想直接了解正確的啟動方式 → [pm2_startup_guide.md](pm2_startup_guide.md)
- 如果想快速排查問題 → [debugging_guide.md](debugging_guide.md)
- 如果想深入理解系統架構 → 從 [案例 1](#案例-1pm2--數據庫配置問題) 開始閱讀

> **重要更新** (2025-12-01):
> - 本文檔已重命名為 `debugging_case_studies.md`,定位為學習資源
> - 操作性除錯指南請查閱 [debugging_guide.md](debugging_guide.md)
> - 禁止直接通過 SQL/SQLite 修改帳戶配置,請使用交互式 CLI
> - 統一使用容器環境變數 `KF_HOME=/app/runtime`
