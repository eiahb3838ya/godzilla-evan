...
if check_pytorch_version('2.4'):
    ...
    if device != 'cuda':
        warnings.warn(
            f"PyTorch {torch.__version__} < 2.4.0 且偵測到非 CUDA 裝置（{device}），"
            "僅啟用 PyTorch fallback，效能可能受限。",
            stacklevel=1,
        )
    autocast_custom_fwd = device_torch_lib.amp.custom_fwd if hasattr(device_torch_lib, 'amp') else torch.cuda.amp.custom_fwd
    autocast_custom_bwd = device_torch_lib.amp.custom_bwd if hasattr(device_torch_lib, 'amp') else torch.cuda.amp.custom_bwd

    def custom_device_ctx(index: int):
        return device_torch_lib.device(index) if device != 'cuda' else torch.cuda.device(index)
